...
# Apply legacy Windows 7 / Ubuntu 18.04 / non-AVX / 32-bit patches
echo "=== Applying legacy OS support patches ==="
for patch in patches/legacy/*.patch; do
    if [ -f "$patch" ]; then
        echo "Applying: $(basename $patch)"
        patch -p1 --forward < "$patch" 2>/dev/null || echo "  (skipped - already applied or incompatible)"
    fi
done
echo "=== Legacy patches application complete ==="
